...
export async function getMapMeta(mapId: number): Promise<MapResponse> {
  try {
    return await apiGet<MapResponse>(`/maps/${mapId}`);
  } catch (err) {
 ...
 */
export async function getCategories(mapId: number): Promise<CategoryResponse[]> {
  try {
    return await apiGet<CategoryResponse[]>(`/maps/${mapId}/categories`);
  } catch {
    try {
